...
export const rectCircleColliding = circle => rect => {
  ...
  if (distX > (rect.width/2 + circle.radius)) { return true }
  if (distY > (rect.height/2 + circle.radius)) { return true }

  if (distX <= (rect.width / 2)) { return false }
  if (distY <= (rect.height / 2)) { return false }
